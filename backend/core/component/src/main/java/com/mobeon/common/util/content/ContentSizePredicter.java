 ...
 * "com.mobeon.common.util.content.ContentSizePredicter.csv" must be present in java home.
...
public class ContentSizePredicter {
    ...
    /**
     * Writes prediction stats to a file named "com.mobeon.common.util.content.ContentSizePredicter.csv"
    ...
    private static void log(long encodededSize, String encoding, String contentType, long actualSize, EncodingStats encodingStats) {
        ...
        if (f.exists()) {
            try {
                ...
                LOGGER.debug("Exception while writing stats to " + f.getName()+ " "+e.getMessage(), e);
            }
        }
    }
