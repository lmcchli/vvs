...
public class M3Utils {
     ...
     */
    public static void lowerCasePropertyNames(Properties props) {
        String name;
        String value;
        Properties tmp = new Properties();
        for (Enumeration e = props.propertyNames(); e.hasMoreElements(); ) {
            name = (String) (e.nextElement());
            // dont owerwrite with lowercase. Otherwise default props might overwrite real props.
            if( !name.equals(name.toLowerCase()) || tmp.getProperty(name.toLowerCase()) == null) {
     ...
     * save the code. It was boring to type.
    private static String [] hexByte= {
    ...
    public static String hexDump(byte[] ba) {
        char b;
        int lineByte = 0;
        StringBuffer sb = new StringBuffer((ba.length + 15) / 16 * startLine.length() + 1);
