...
public class SipFactory {
	...
	public synchronized SipStack createSipStack(Properties properties)
		...
		if ( ipAddress == null) {
			SipStack mySipStack = this.sipStackByName.get(name);
			if (mySipStack == null) {
			...
			for (i = 0; i < sipStackList.size(); i++) {
                if (sipStackList.get(i).getIPAddress() != null) {
                    if ((sipStackList.get(i)).getIPAddress().equals( ipAddress )) {
                        return sipStackList.get(i);
                    }
                }
            }
