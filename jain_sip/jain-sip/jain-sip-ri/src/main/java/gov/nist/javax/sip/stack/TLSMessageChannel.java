...
public final class TLSMessageChannel extends MessageChannel implements
    ...
    private static final Logger log = Logger.getLogger(TLSMessageChannel.class);
    ...
    private final TLSMessageProcessor tlsMessageProcessor;
    ...
    protected TLSMessageChannel(Socket sock, TLSMessageProcessor msgProcessor)
            throws IOException {
        ...
        if (log.isDebugEnabled())
            log.debug("Creating new TLSMessageChannel-" + myId);
        ...
        key = MessageChannel.getKey(peerAddress, peerPort, ListeningPoint.TLS);
        ...
        myThread.setName("TLSMessageChannelThread-" + myId);
     ...
     */
    protected TLSMessageChannel(InetAddress peerAddress, int peerPort,
        ...
        key = MessageChannel.getKey(this.peerAddress, this.peerPort, ListeningPoint.TLS);

        if (log.isDebugEnabled())
            log.debug("Creating new TLSMessageChannel-" + myId + " " + key);
    ...
    public void sendMessage(SIPMessage sipMessage) throws IOException {
        ...
        if (getMessageProcessor().getListeningPoint().getSipStack().serverLog.needsLogging(ServerLog.TRACE_MESSAGES))
...
//            this.peerProtocol = v.getTransport();
    ...
    public void run() {
        ...
        PipelinedMsgParser myParser = new PipelinedMsgParser(this, in,
                this.getMessageProcessor().getListeningPoint().getSipStack()
                .getMaxMessageSize());
    ...
    protected void uncache() {
        tlsMessageProcessor.uncacheChannel(this);
    }
