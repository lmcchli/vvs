...
// ANOTHER SOLUTION MUST BE DETERMINED BY MAS.
// TEMPORARY FIX will only work for incoming call and not outdial_notification.
...
function Event_SendMessageStateEvent(typeOfEvent, subscriber, messageId, ani) {
	...
	var eventString =  "username="        + subscriber +
	...
	System_SendTrafficEvent(eventName, eventString);
	return eventString;
}
...
function Greeting_Activate(phoneNumber, greetingType, mediaType) {
	...
	activeGreetings = activeGreetings + "," + greetingType;
        mas.subscriberSetStringAttribute(phoneNumber, "activegreetingid", [activeGreetings]);
    }
}
/**
...
function Login_IsOwnPhoneCheckOk(subscriber, ani){
    if (subscriber == ani)
	    ...
	    return true;
    else if (mas.subscriberGetIntegerAttribute(subscriber, "userlevel")[0] >= 10)
	    return true;
    else
        return false;
}
...
function Login_ShallPreferredLanguageBeSet(phoneNumber) {
    if (mas.systemGetConfigurationParameter("vva.dialogbehaviour", "loginsetlanguages") == "no")
        ...
        return true;
    else if (mas.subscriberGetIntegerAttribute(phoneNumber, "userlevel")[0] >= 5)
        return false;
    else
        return true;
}
 ...
 */
function Mailbox_GetMaxRecordingLength(phoneNumber, callerPhoneNumber) {
    var usageId = mas.mailboxUsage(mas.subscriberGetMailbox(phoneNumber));
    ...
    var secondsLeft = Utility_SecondsLeftOfMessageRecording(phoneNumber, callerPhoneNumber);
    ...
    if (byteQuota != -1 && secondsLeft <= endOfMsgWarning+secondsOverhead)
	return 0;
    else if ( byteQuota != -1 && secondsLeft <= maxLengthOfMsg+secondsOverhead)
	return secondsLeft;
    else
...
function Mailbox_IsFilledOverWarningLevel(phoneNumber) {
    ...
    var byteQuota = mas.subscriberGetIntegerAttribute(phoneNumber, "mailquota")[0];
    var bytesUsed = mas.mailboxGetByteUsage(usageId);

    //Check if the disk usage is over byte quota warning
    if (byteQuota != -1 && (bytesUsed >= (1 - warningLevel) * byteQuota))
	return true;
    else
	return false;
}
...
function Mailbox_IsFull(phoneNumber) {
    ...
    //Check if the disk usage is over byte quota
    if (byteQuota != -1 && bytesUsed >= byteQuota)
	return true;
    else
	return false;
}
 ...
 * Returns only the string from # and forward if the document of current and previous are equal.
 */
...
function Message_CheckRecepients(emailArray, mediaType) {
	...
	phoneNumber = mas.systemGetSubscribers("mail", emailArray[i]);
	    ...
	    newEmailArray.push(emailArray[i]);
    }
...
/**
 * #DisplayName Message Create New Deposit
 * #Description Returns the identity of the message.
 * #param sender sender The phone number of the sender.
 * #param recipient recipient The phone number of the subscriber that recieves the message.
 * #param mediaObject mediaObject The message that was recoreded.
 * #param mediaType mediaType The type of the message, ie voice/video/fax/email
 ...
 */
function Message_CreateNewDeposit(sender, recipient, mediaObject, mediaType, spokenName) {
    var from;
    var subject = "";
    ...
    var subjectForwarded = "";
    var subjectReply = "";
    var subjectConfidential = "";
    var subjectVoice = "";
    var subjectVideo = "";
    var subjectFax = "";
    var subjectEmail = "";
    var subjectSpokenName = "";
    var subjectFrom = "";
    var subjectName = "";
    var senderUnknown = "";
    var senderRestricted = "";

    var messageId = mas.messageCreateNew();

    // get subcriber data
    var emailAddress = mas.subscriberGetStringAttribute(recipient, "mail")[0];
    var preferredLanguage = mas.subscriberGetStringAttribute(recipient, "preferredlanguage")[0];
    var subscriberName = mas.subscriberGetStringAttribute(recipient, "cn")[0];
	...
	senderEmailAddress = mas.subscriberGetStringAttribute(sender, "mail")[0];
	senderName = mas.subscriberGetStringAttribute(sender, "cn")[0];
	includeSpokenName = mas.subscriberGetBooleanAttribute(sender, "includespokenname")[0];
	depositNoQuotaCheck = mas.systemGetConfigurationParameter("vva.dialogbehaviour", "depositnoquotacheck");
	subjectFrom == "person";
	subjectName = senderName;

        // ANI is not restricted, set from field
	if (Call_GetAniPi() == 0)
	    from = subscriberName + " (" + sender + ") <" + senderEmailAddress + ">";
	else
	    from = subscriberName + "<" + senderEmailAddress + ">";
    }
	...
	// check if call restriction and original ANI.
	if (Call_GetAniPi() == 1) {
	    from = "Number Restricted <>";
	    //from = util.convertMediaObjectsToString(mas.systemGetMediaContent("Prompt", "restricted"));
	}
	else  if (Call_GetAni() != "") {
	    from = "Unknown Caller <>";
	    //from = util.convertMediaObjectsToString(mas.systemGetMediaContent("Prompt", "unknown"));
	}
	else
	    from = sender + " <>";
    }

    subject = "Voice message from " + senderName;
/*
    if (mediaType == "voice") {
	subjectVoice = util.convertMediaObjectsToString(mas.systemGetMediaContent("Prompt", "voice"));
	subjectFrom = util.convertMediaObjectsToString(mas.systemGetMediaContent("Prompt", subjectFrom));
	subject = subjectVoice + " " + subjectFrom + " " + "Martin";
    }
  */
    Message_SetProperty(messageId, "sender", [from]);
    Message_SetProperty(messageId, "recipients", [emailAddress]);
    Message_SetProperty(messageId, "subject", [subject]);
    Message_SetProperty(messageId, "type", [mediaType]);
    Message_SetProperty(messageId, "language", [preferredLanguage]);

    if (spokenName != "")
        mas.messageSetSpokenNameOfSender(messageId, spokenName, "Originators spoken name", "spoken.wav", preferredLanguage);

    mas.messageAddMediaObject(messageId, mediaObject, "Mobeon voice message", "message.wav", preferredLanguage);

    return messageId;
}

/**
 * #DisplayName Message Create New Retrieval
 * #Description Returns the identity of the message.
 * #param sender sender The phone number of the sender.
 * #param toList toList A list of email-addresses of recipients.
 * #param ccList ccList A list of email-addresses of recipients used as secondary recipients.
 * #param mediaObject mediaObject The message that was recoreded.
 * #param mediaType mediaType The type of the message, ie voice/video/fax/email
 * #param spokenName spokenName The spoken name of the caller that will be attached to the message. Empty if no spoken name should be attached.
 */
function Message_CreateNewRetrieval(sender, toList, ccList, mediaObject, mediaType, spokenName) {
...
function Message_DaysSinceDelivery(mailboxId, messageId) {
	var currentTime = getCurrentTime(mas.subscriberGetStringAttribute(mailboxId, "subscribertimezone")[0]);
	var deliveryDateInSeconds =  util.formatTime(mas.messageGetProperty(messageId, "deliverydate"), null);
...
function Message_IsPartText(messageContentId) {
	if (messageGetMediaProperties(messageContentId).search("text") != -1) {
		return true
	}
	return false
}
...
function Message_IsRespondToSenderAllowed(mailboxId, messageId) {
	...
	if (mas.messageGetProperty(messageId, "deliveryreport")[0] == "false")
		return false;

	// Check if fax that is not forwarded
	if ((mas.messageGetProperty(messageId, "type")[0] == "fax") && (mas.messageGetProperty(messageId, "forwarded")[0] == "false"))
		return false;
...
function Message_PlayEmailHeader(messageId) {
	...
	objectArray.push(util.getMediaObject(Message_GetProperty(messageId, "subject"))); //<Subject>
	return objectArray;
}
...
function Message_SetTTSLanguage(subscriber, mediaObject) {
	...
	for (i=0;i<installedLang.length;i++) {
...
function PhoneNumber_GetAnalyzedNumber(rule, phoneNumber, callerPhoneNumber) {
    if (phoneNumber == undefined)
        return "";
// REMOVE COMMENTS WHEN callerPhoneNumber IS SUPPORTED BY MAS!!!!!!!!!!!!!
...
function SMS_SendSlamdownInformation(phoneNumber, ani) {
    ...
    if (Call_GetAni() == "" || Call_GetAniPi() == 1 || Subscriber_HasService(phoneNumber, "slamdown_notification") == false)
	var a = 1;
    ...
    // THIS IS NOT CLEAR WHAT SHOULD BE SENT
    System_SendTrafficEvent("slamdowninformation", "mailhost=" + Subscriber_GetSingleStringAttribute(phoneNumber, "mailhost") +
			    ",emailaddress=" + Subscriber_GetSingleStringAttribute(phoneNumber, "mail") + ",sender=" + number);
	return "slamdowninformation", "mailhost=" + Subscriber_GetSingleStringAttribute(phoneNumber, "mailhost") +
			    ",emailaddress=" + Subscriber_GetSingleStringAttribute(phoneNumber, "mail") + ",sender=" + number;
}
...
function Subscriber_HasService(phoneNumber, service)
{
	...
	if ((service == "prerecordedgreeting_tui") && (serviceDn[i].search("prerecordedgreeting_tui") != -1))
	   return true;
        ...
        if (service == "mwi") {
	    // Return true if mwi_notification service is active and MWI is not disabled in emntd.
            if ((serviceDn[i].search("mwi_notification") != -1)) {
                if (mas.subscriberGetStringAttribute(phoneNumber, "emntd")[0].indexOf('MWI') == -1)
		    return true;
	   }
    	}
        if (service == "slamdown_notification") {
	    // Return true if slamdown_notification service is active and slamdown is not disabled in emusersd.
            if ((serviceDn[i].search("slamdown_notification") != -1)) {
                if (mas.subscriberGetStringAttribute(phoneNumber, "emusersd")[0].indexOf('slamdown_notification') == -1)
		  return true;
	   }
    	}
    }
 ...
 */
function System_SetOffset(mediaObject, offset) {
	return util.systemSetOffset(mediaObject, offset);
}
...
/**
 * Internal function.
 * Calculates the number of seconds that is left for a message recording.
*/
function Utility_SecondsLeftOfMessageRecording(phoneNumber, callerPhoneNumber)  {
    var usageId = mas.mailboxUsage(mas.subscriberGetMailbox(phoneNumber));
    var bytesUsed = mas.mailboxGetByteUsage(usageId);
    var byteQuota = mas.subscriberGetIntegerAttribute(phoneNumber, "mailquota")[0];

    var includeSpokenName = false;
    var maxLengthOfGreeting = 0;
    var secondsLeft;
    var bitrate;
    ...
    // if caller is a subscriber
    if (PhoneNumber_IsASubscriber(callerPhoneNumber)) {
    ...
    if (includeSpokenName == true)
        secondsLeft = (((byteQuota - bytesUsed -50000)*8.0/1000) / bitrate/4*3.0) - (maxLengthOfGreeting/3*4.0) + 0.5;
    else
        secondsLeft = (((byteQuota - bytesUsed -50000)*8.0/1000) / bitrate/4*3.0) + 0.5;
...
var GlobalObject = { ANI:'', Subscriber:'', LoggedIn:false, LoginType:'retrieval', MainMenuValid:false, PreviousMenuValid:false };
...
function PlatformAccess() {
    ...
    this.messageSetSpokenNameOfSender = messageSetSpokenNameOfSender;
...
function mailboxUsage(mailboxId) {
    return 1;
}

function mailboxGetMessageUsage(usageId) {
    return 3;
}

function mailboxGetByteUsage(usageId) {
    return 500;
}
...
function messageAddMediaObject(messageId, mediaObject, description, filename, language) {
    return true;
}
...
function messageSetSpokenNameOfSender(messageId, spokenName, description, filename, language) {
return true;
}
...
function subscriberGetStringAttribute(phoneNumber, attrName) {
    ...
    if(attrName == "cn")
    	return ["Martin"];
    if(attrName == "diskspaceremainingwarninglevel")
	return [".3"];
    if(attrName == "emftl")
	...
	if (attrName == "emtuiblockedmenu")
		return ["NoAnswerGreeting", "BusyGreeting", "OutOfHoursGreeting", "distributionlist", "attcallcoverage", "notification", "messageplayoptions", "timeformat", "defaultfaxnumber", "setpreferredlanguagemenu" ];
...
function systemGetConfigurationParameter(group, parameterName) {
	if (parameterName == "systemunpauseanykey")
		return "no";
		...
		return "3";
	else if (parameterName == "minrecmsg")
		return "1000";
	else if (parameterName == "minrecnotmsg")
		return "1000";
	else if (parameterName == "redirectcausenoslamdown")
		return "[noanswer]";
	else
...
function systemGetMediaContent(type, mediaContentId, qualifier) {
	...
	if (mediaContentId == 2205)
		return "Media/English/voice/VVA_1005.wav";
	if (mediaContentId == 2207)
		return "Media/English/voice/VVA_1007.wav";
	if (mediaContentId == 2208)
		return "Media/English/voice/VVA_1008.wav";
	if (mediaContentId == 2209)
		return "Media/English/voice/VVA_1009.wav";
...
function PlatformAccessUtil() {
    this.convertMediaObjectsToString = convertMediaObjectsToString;
...
function convertMediaObjectsToString(mediaObject) {
...
function getMediaObjectProperty(mediaObject, propertyName) {
...
function setMediaObjectProperty(mediaObject, propertyNames, propertyValues) {
}
...
var remote = { number:'999999', pi:'0'};
var local = { number:'222222'};
var calltype = "voice";
var redirect = new Array({number:'', pi:'0', reason:'unknown'});
var ccxml = { namelist:'incoming_call'};
