...
function getVoiceMsgMedia(folder, id) {
    ...
    try {
        ...
        folder.getMessageList(inboxMsgList, type, flagsAll, Message.FIFO);
        ...
        var bodyList =  msg.getHeader(Message.BODY);
        print ("Getting body");
        var body = bodyList.elementAt(0); // Get the first body only
        print ("Getting media");
        media =   msg.getBody(body, Message.MEDIA);
        if (media == null) {
            print ("Media is null!");
        }
...
function storeMsg(msgMedia) {
    // STORE MESSAGE
    var depositMsg = new Message();
    var to = new Vector();
    var bodyList = new Vector();
    var mediaList = new Vector();
    var cal = new GregorianCalendar();
    try {
        depositMsg.setValue(Message.TYPE, new Integer(Message.VOICE));
        depositMsg.setValue(Message.FROM, MAS_A_NUM);
        to.addElement(MAS_C_NUM);
        depositMsg.setValue(Message.TO, to);
        depositMsg.setValue(Message.DATE, cal.getTime().toString());
        depositMsg.setValue(Message.PHONE, MAS_C_NUM);
        bodyList.addElement("voiceMsgBody 1");
        depositMsg.setValue(Message.BODY, bodyList);
        mediaList.addElement(msgMedia);
        depositMsg.setValue(Message.MEDIA, mediaList);
        depositMsg.setValue(Message.SUBJECT, "Voice message from MAS");
    } catch(dataexception) {
        print("DataException: " + dataexception.toString());
    }
    try {
        depositMsg.send();
    }
    catch(e) {
        print ("Exception caught while storing message");
    }
}
