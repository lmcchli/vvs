...
/**
 * This class contains test cases where a SIP message is split over multiple
 * TCP packets.
 * @author Malin Nyfeldt
 */
public class SplitMessageTest extends TransportCase {
    ...
    /*
    public void testInboundCallWhereAckIsSplit() throws Exception  {
        ...
        String parsedAck = ack.getParsedMessage();
        tcpClient.send(parsedAck.substring(0, parsedAck.length() / 2));
        Thread.sleep(300);
        tcpClient.send(parsedAck.substring(
                parsedAck.length() / 2 + 1, parsedAck.length()));
