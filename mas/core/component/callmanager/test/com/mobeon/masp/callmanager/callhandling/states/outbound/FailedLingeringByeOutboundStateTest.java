...
/**
 * FailedLingeringByeOutboundState Tester.
 *
 * @author Malin Flodin
 */
public class FailedLingeringByeOutboundStateTest extends OutboundStateCase {
    FailedLingeringByeOutboundState failedLingeringByeState;

    protected void setUp() throws Exception {
        super.setUp();

        failedLingeringByeState = new FailedLingeringByeOutboundState(
    ...
    public void testDial() throws Exception {
        ...
        failedLingeringByeState.dial(dialEvent);
    }
    ...
    public void testSendToken() throws Exception {
        assertNotAllowedEvent(mockOutboundCall,
                "SendToken is not allowed in Failed state " +
                        "(sub state LingeringBye).");
        failedLingeringByeState.sendToken(sendTokenEvent);
    }
     ...
     * {@link FailedCompletedOutboundState} if the response method is BYE.
     *
     * @throws Exception if test case failed.
     */
    public void testProcessResponseForBye() throws Exception {
        assertResponseCodeRetrievedForMethod("BYE", 600);
        assertStateFailed(FailedOutboundState.FailedSubState.COMPLETED);
        failedLingeringByeState.processSipResponse(sipResponseEvent);
    }
    ...
    public void testHandleCallTimeout() throws Exception {
        mockOutboundCall.expects(never());
        failedLingeringByeState.handleCallTimeout(
