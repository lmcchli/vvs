...
/**
 * ProgressingCallingOutboundState Tester.
 *
 * @author Malin Flodin
 */
public class ProgressingCallingOutboundStateTest extends OutboundStateCase {
    ProgressingCallingOutboundState progressingCallingState;

    protected void setUp() throws Exception {
        super.setUp();

        progressingCallingState = new ProgressingCallingOutboundState(
                (OutboundCallInternal) mockOutboundCall.proxy());
    }

    /**
     * Verifies that an lock request in state
     * {@link ProgressingCallingOutboundState} results in setting the state to
     * {@link FailedWaitingForResponseOutboundState} since a CANCEL or BYE
     * cannot be sent until a response has been received for the INVITE.
    ...
    public void testLock() throws Exception
    {
        ...
        assertStateFailed(FailedOutboundState.FailedSubState.WAITING_FOR_RESPONSE);
        assertStreamsDeleted(mockOutboundCall);
        progressingCallingState.processLockRequest();
    }
    ...
    public void testPlay() throws Exception {
        ...
        progressingCallingState.play(playEvent);
    }
    ...
    public void testRecord() throws Exception {
        ...
        progressingCallingState.record(recordEvent);
    }
    ...
    public void testStopPlay() throws Exception {
        ...
        progressingCallingState.stopPlay(stopPlayEvent);
    }
    ...
    public void testStopRecord() throws Exception {
        mockOutboundCall.expects(never());
        progressingCallingState.stopRecord(stopRecordEvent);
    }

    /**
     * Verifies that a video fast update request in
     * {@link ProgressingCallingOutboundState} is ignored.
     * @throws Exception if test case fails.
     */
    public void testProcessVideoFastUpdateRequest() throws Exception {
        mockOutboundCall.expects(never());
        progressingCallingState.processVideoFastUpdateRequest();
    }

    /**
     * Verifies that a disconnect request in state
     * {@link ProgressingCallingOutboundState} results in setting the state to
     * {@link FailedWaitingForResponseOutboundState} since a CANCEL or BYE
     * cannot be sent until a response has been received for the INVITE.
     * A {@link FailedEvent} is generated.
    ...
    public void testDisconnect() throws Exception {
        ...
        assertStateFailed(FailedOutboundState.FailedSubState.WAITING_FOR_RESPONSE);
        assertStreamsDeleted(mockOutboundCall);
        progressingCallingState.disconnect(disconnectEvent);
    }
    ...
    public void testDial() throws Exception {
        assertNotAllowedEvent(mockOutboundCall,
                "Dial is not allowed in Progressing state " +
                        "(sub state Calling).");
        progressingCallingState.dial(dialEvent);
    }
    ...
    public void testSendToken() throws Exception {
        assertNotAllowedEvent(mockOutboundCall,
                "SendToken is not allowed in Progressing state " +
                        "(sub state Calling).");
        progressingCallingState.sendToken(sendTokenEvent);
    }
    ...
    {
        mockOutboundCall.expects(never());
        progressingCallingState.processAck(additionalSipRequestEvent);
    }
    ...
    public void testProcessBye() throws Exception {
        ...
        assertStreamsDeleted(mockOutboundCall);
        progressingCallingState.processBye(additionalSipRequestEvent);
    }
    ...
    public void testProcessByeWithReason() throws Exception {
        assertReasonHeader(mockAdditionalRequest, 40, 1);
        assertFailedEvent(mockOutboundCall,
                FailedEvent.Reason.REJECTED_BY_FAR_END, CallDirection.OUTBOUND,
                "Call rejected by far end with SIP response code 480", 620);
        assertOkResponseSent(mockOutboundCall);
        assertStateFailed(FailedOutboundState.FailedSubState.COMPLETED);
        assertStreamsDeleted(mockOutboundCall);
        progressingCallingState.processBye(additionalSipRequestEvent);
    ...
    public void testProcessCancel() throws Exception {
        mockOutboundCall.expects(never());
        progressingCallingState.processCancel(additionalSipRequestEvent);
    }
    ...
    public void testProcessInvite() throws Exception {
        try {
            mockOutboundCall.expects(never());
            progressingCallingState.processInvite(initialSipRequestEvent);
    ...
    public void testProcessReInvite() throws Exception
    {
        ...
        progressingCallingState.processReInvite(additionalSipRequestEvent);
    }
    ...
    {
        assertOkResponseSent(mockOutboundCall);
        progressingCallingState.processOptions(additionalSipRequestEvent);
    }
    ...
    public void testProcessInfo() throws Exception
    {
        ...
        progressingCallingState.processInfo(additionalSipRequestEvent);
    }
    ...
    public void testProcessPrack() throws Exception
    {
        ...
        progressingCallingState.processPrack(additionalSipRequestEvent);
    }

    /**
     * Verifies that a SIP Trying response results in state
    ...
    public void testProcessTryingResponse() throws Exception {
        ...
        assertStateProgressing(
                ProgressingOutboundState.ProgressingSubState.PROCEEDING);
        progressingCallingState.processSipResponse(sipResponseEvent);
    }

    /**
     * Verifies that a SIP "Call Is Being Forwarded" response results in state
     * {@link ProgressingProceedingOutboundState}.
     * @throws Exception if test case fails.
     */
    public void testProcessCallIsBeingForwardedResponse() throws Exception {
        assertResponseCodeRetrievedForMethod(
                "INVITE", Response.CALL_IS_BEING_FORWARDED);
        assertStateProgressing(
                ProgressingOutboundState.ProgressingSubState.PROCEEDING);
    ...
    /**
     * Verifies that a SIP Queued response results in state
     * {@link ProgressingProceedingOutboundState}.
     * @throws Exception if test case fails.
     */
     ...
     * indicating NO early media and that state is set to
     * {@link ProgressingProceedingOutboundState}.
     * @throws Exception if test case fails.
     */
     ...
     * CANCEL request is sent when the SDP answer could not be parsed.
     ...
     * @throws Exception if test case fails.
     */
    public void testProcessSessionProgressResponseWithNoFoundSdpIntersection() throws Exception {
        assertResponseCodeRetrievedForMethod("INVITE", Response.SESSION_PROGRESS);
        assertParsingSdpBody(mockOutboundCall, SUCCEED);
    ...
    /**
     * Verifies that a reliable SIP Ringing response results in a
     * {@link com.mobeon.masp.callmanager.events.ProgressingEvent} generated
     * indicating NO early media, the state set to
     * {@link ProgressingProceedingOutboundState} and a PRACK request sent.
     * @throws Exception if test case fails.
     */
    public void testProcessReliableRingingResponse() throws Exception {
        assertResponseCodeRetrievedForMethod("INVITE", Response.RINGING);
        assertProgressingEvent(mockOutboundCall, NO_EARLY_MEDIA);
        assertStateProgressing(
                ProgressingOutboundState.ProgressingSubState.PROCEEDING);
    ...
    /**
     * When a reliable SIP Ringing response is received it results in a
     * {@link com.mobeon.masp.callmanager.events.ProgressingEvent} generated
     * indicating NO early media, the state set to
     * {@link ProgressingProceedingOutboundState} and a PRACK request sent.
     ...
     * @throws Exception if test case fails.
     */
    public void testProcessReliableRingingResponseWhenPrackCannotBeSent()
            throws Exception {
        assertResponseCodeRetrievedForMethod("INVITE", Response.RINGING);
        assertProgressingEvent(mockOutboundCall, NO_EARLY_MEDIA);
        assertStateProgressing(
    ...
    /**
     * Verifies that when receiving a SIP OK response a SIP
     * BYE request is sent when the call types differ.
     * A {@link FailedEvent} is generated and the
     * state is set to {@link FailedLingeringByeOutboundState}.
     * @throws Exception if test case fails.
     */
    public void testProcess2xxResponseWhenNoSdpIntersectionFoundDueToCallType()
            throws Exception {
        assertResponseCodeRetrievedForMethod("INVITE", Response.OK);
        assertCreateAck(SUCCEED);
        assertSendRequestWithinDialog(SUCCEED);
    ...
    /**
     * Verifies that a SIP 3xx response when retrieving a new contact results
     * in the call considered rejected. A {@link FailedEvent} is
     * generated and the state is set to {@link FailedCompletedOutboundState}.
     * @throws Exception if test case fails.
     */
    public void testProcess3xxResponseWhenRetrivingNewContactFails()
            throws Exception {
        assertReasonHeader(mockResponse, 40, 1);
        assertResponseCodeRetrievedForMethod("INVITE", 300);
        assertRedirectionAllowed();
        assertCallNotAlreadyRedirected();
        assertContactsRetrieved();
        assertNextContact((SipURI)mockedSipURI.proxy(), FAIL);
        assertFailedEvent(mockOutboundCall,
                FailedEvent.Reason.REJECTED_BY_FAR_END, CallDirection.OUTBOUND,
                "Call rejected by far end with SIP response code 300", 620);
        assertStateFailed(FailedOutboundState.FailedSubState.COMPLETED);
    ...
    public void testProcessSipTimeoutWhenNoNewRemoteParty() throws Exception {
        ...
        assertStateFailed(FailedOutboundState.FailedSubState.COMPLETED);
        assertStreamsDeleted(mockOutboundCall);
        progressingCallingState.processSipTimeout(sipTimeoutEvent);
    ...
    /**
     * Verifies that if the "Not Connected" timer specified by
     * the client expires in this state results in a failed event and the
     * state {@link FailedWaitingForResponseOutboundState}.
     *
     * @throws Exception if test case failed.
     */
    public void testHandleMaxDurationBeforeConnectedCallTimeout() throws Exception {
        assertFailedEvent(mockOutboundCall,
                ...
                "The \"max duration before connected\" timer has " +
                        ...
                        "The call will be disconnected (with a CANCEL or BYE) " +
                        "as soon as a SIP response has been received",
                621);
        assertStateFailed(FailedOutboundState.FailedSubState.WAITING_FOR_RESPONSE);
        assertStreamsDeleted(mockOutboundCall);
        progressingCallingState.handleCallTimeout(
    ...
    public void testHandleOtherCallTimeout() throws Exception {
        mockOutboundCall.expects(never());
        progressingCallingState.handleCallTimeout(maxCallDurationTimeoutEvent);
    }
    ...
    public void testHandleAbandonedStream() throws Exception {
        mockOutboundCall.expects(never());
        progressingCallingState.handleAbandonedStream();
    }
