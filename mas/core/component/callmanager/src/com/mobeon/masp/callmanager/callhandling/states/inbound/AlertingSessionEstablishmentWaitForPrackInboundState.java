...
/**
 * Represents the sub state "SessionEstablishment, wait for PRACK" of {@link AlertingInboundState}.
 * <p>
 * This sub state is entered when:
 * <ul>
 * <li> Call Manager has precondition and/or unicast validation enabled and
 * <li> Call Manager has detected that preconditions and/or unicast conditions MUST be fulfilled by UAC
 * </ul>
 ...
 */
public class AlertingSessionEstablishmentWaitForPrackInboundState extends AlertingInboundState {

    private final ILogger log = ILoggerFactory.getILogger(getClass());

    public AlertingSessionEstablishmentWaitForPrackInboundState(InboundCallInternal call) {
        super(call);
    }

    public String toString() {
        return "Alerting state (sub state SessionEstablishmentWaitForPrack)";
    }

    /**
     * This method is used when the Call Manager client accepts the inbound call while 
     * Call Manager is waiting for a PRACK request from UAC.
     * <ul>
     * <li>
     * Buffer the accept until the PRACK request is received {@link #sendOkResponse(String)}.
     * </li>
     * <li>
     ...
     * The initial inbound INVITE is rejected with a SIP "Temporarily Unavailable" response.
     * <p>
     * If the SIP error response could not be sent, an {@link ErrorEvent} is generated and the state is set to {@link ErrorInboundState}.
     */
    ...
    /**
     * This method is used when the Call Manager client rejects the inbound call.
     * <p>
     * A {@link FailedEvent} is generated and the state is set to {@link FailedCompletedInboundState}.
     * A SIP "Forbidden" response is sent for the initial INVITE.
     * <p>
     * If the SIP response could not be sent, an {@link ErrorEvent} is generated and the state is set to {@link ErrorInboundState}.
     *
     ...
     * <p>
     * If a SIP response could not be sent, an {@link ErrorEvent} is generated and the state is set to {@link ErrorInboundState}.
     *
    ...
    /**
     * This method is used when a SIP 183 "Session Progress" response has been sent reliably and Call Manager receives a PRACK request for that response.
     * 
     * <p>
     * If the SIP PRACK does not fulfill preconditions and/or unicast, the state is set to {@link AlertingSessionEstablishmentInboundState}
     * in order to wait for subsequent SIP UPDATE from UAC.
     * 
     * <p>
     * If the SIP PRACK fulfill preconditions and/or unicast:
     * <ul>
     * <li> if earlyMedia was requested by Call Manager client, the state is set to {@link AlertingEarlyMediaInboundState} and a {@link EarlyMediaAvailableEvent} is generated.
     * <li> if earlyMedia was not requested by Call Manager client, the state is set to {@link AlertingAcceptingInboundState}.
     * </ul>
     * 
     * <p>
     * A SIP "OK" response is sent for the PRACK using {@link #sendOkResponse(SipRequestEvent, String)}.
     * 
    ...
    public synchronized void processPrack(SipRequestEvent sipRequestEvent) {
        ...
        try {
            if (call.containsSdp(sipRequestEvent)) {
                if (log.isDebugEnabled())
                    log.debug("A new remote SDP offer received in PRACK");
                ...
                if (log.isDebugEnabled())
                    log.debug("No remote SDP offer received in PRACK");
            }
            ...
            if (log.isDebugEnabled())
                log.debug("OK response is sent for PRACK (" + (sdpAnswer == null ? "No ":"") + "SDP answer included)");
            ...
            if (preconditionsFulfilled && unicastFulfilled) {
                ...
                // Go to Alerting Session Establishment state
                call.setStateAlerting(AlertingSubState.SESSION_ESTABLISHMENT);
            }

        } catch (PreconditionException pe) {
            // Need to send a SIP OK response to a PRACK that 'acks' an existing 183
            // note: omit the sdpAnswer, as we didn't fill it up properly because of the exception.
            sendOkResponse(sipRequestEvent, null);
            
            // But still, we have a precondition exception - so we can't continue and will tear down call.
            ...
            // But still, we have a unicast exeption - so we can't continue and will tear down call.
     ...
     * <p>
     * An {@link ErrorEvent} is generated, the state is set to {@link ErrorCompletedInboundState} and a SIP 504 "Server Timeout"
    ...
    public synchronized void processSipTimeout(SipTimeoutEvent sipTimeoutEvent) {
        ...
        call.sendErrorResponse(Response.SERVER_TIMEOUT, call.getInitialSipRequestEvent(), "Reliable provisional response was not acknowledged in time.");
    }
    ...
    public synchronized void handleCallTimeout(CallTimeoutEvent callTimeoutEvent) {
        if (log.isDebugEnabled())
            log.debug("A call timeout " + callTimeoutEvent.getType() + " received in " + this + ".");
        ...
        if (callTimeoutEvent.getType() == CallTimerTask.Type.NO_ACK) {
            ...
            String message = "The inbound call conditions were not fulfilled in time. It is considered abandoned and a SIP 408 response will be sent.";
            if (log.isInfoEnabled())
                log.info(message);
