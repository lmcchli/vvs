...
/**
 * Represents the sub state "Wait for New Media" of {@link AlertingInboundState}.
 * This sub state is entered when the inbound call has been accepted, 
 * an ACK has been received and the support for redirected RTP is activated and 
 * the call media has been put on hold.
 * <p>
 * In this state the initial INVITE has been setup completely, the media streams
 * have been put on hold and the call is waiting for a re-INVITE with new media 
 * settings.
 * <p>
 ...
 */
public class AlertingWaitForNewMediaInboundState extends AlertingInboundState {

    private final ILogger log = ILoggerFactory.getILogger(getClass());

    public AlertingWaitForNewMediaInboundState(InboundCallInternal call) {
        super(call);
    }

    public String toString() {
        return "Alerting state (sub state WaitForNewMedia)";
    }
        ...
        if (call.containsSdp(sipRequestEvent)) {
            ...
            try {
                ...
                if (sdpIntersection == null) {
                    ...
                    call.setStateAlerting(AlertingSubState.WAIT_FOR_NEW_MEDIA_ACK);
                }                
                ...
                if (log.isDebugEnabled())
                    log.debug("Could not create SDP answer or outbound stream.", e);

                if (log.isInfoEnabled()) log.info(
                        "Could not create SDP answer or outbound stream: " + e.getMessage() +
                        ". A SIP 500 response will be sent.");

                // Send Server Internal Error response
                call.sendErrorResponse(Response.SERVER_INTERNAL_ERROR,
                        sipRequestEvent, "Could not create SDP answer or outbound stream.");
                ...
                // Send Server Internal Error response
                if (log.isDebugEnabled())
                    log.debug("Could not create outbound media stream.", e);

                if (log.isInfoEnabled()) log.info(
                        "Could not create outbound media stream: " + e.getMessage() +
                        ". A SIP 500 response will be sent.");

                // Send Server Internal Error response
                call.sendErrorResponse(Response.SERVER_INTERNAL_ERROR,
                        sipRequestEvent, "Could not create outbound media stream answer.");
     ...
     * <p/>
     * If the call timeout is of type {@link CallTimerTask.Type.REDIRECTED_RTP}, 
