...
/**
 * Represents the sub state "LingeringCancel" of {@link FailedOutboundState}.
 * This sub state is entered when an invalid Session Progress response was
 * received for the initial INVITE in {@link ProgressingOutboundState}.
 * A SIP CANCEL request is sent in that situation and a SIP OK response is expected.
 * <p>
 * In this state sendToken is not allowed. A disconnect
 * request generates a {@link com.mobeon.masp.callmanager.events.DisconnectedEvent}.
 * <p>
 * All methods are synchronized to handle each event atomically.
 *
 * @author Malin Flodin
 */
public class FailedLingeringCancelOutboundState extends ErrorOutboundState {

    private final ILogger log = ILoggerFactory.getILogger(getClass());

    public FailedLingeringCancelOutboundState(OutboundCallInternal call) {
        super(call);
    }

    public String toString() {
        return "Failed state (sub state LingeringCancel)";
    }
     ...
     * INVITE, the state is set to {@link FailedCompletedOutboundState}.
     * If the response is a final ok response (i.e. 2xx) to the initial INVITE,
     * a SIP ACK request is sent, the state is set to
     * {@link FailedLingeringByeOutboundState} and a SIP BYE request is sent.
    ...
    public synchronized void processSipResponse(SipResponseEvent sipResponseEvent) {
        ...
        responseCode = sipResponseEvent.retrieveResponseCodeForMethod(Request.INVITE);
        ...
        if (responseCode != null) {
            ...
            if (responseType == 1) {
                ...
                try {
                    ...
                    call.setStateFailed(
                            FailedOutboundState.FailedSubState.LINGERING_BYE);
            ...
            // Check for response to CANCEL
            responseCode = sipResponseEvent.retrieveResponseCodeForMethod(Request.CANCEL);

            if (responseCode != null) {
                // A response for CANCEL is received. igonring answer and set to COMPLETE.
                if (log.isDebugEnabled())
                    log.debug("Processing CANCEL response");
                if (log.isInfoEnabled()) log.info("Call is disconnected.");
                call.setStateFailed(FailedOutboundState.FailedSubState.COMPLETED);
            }
        }
    }
