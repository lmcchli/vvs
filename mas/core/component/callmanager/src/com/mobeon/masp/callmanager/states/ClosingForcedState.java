...
/**
 * The administrative state forced Closing.
 *
 * @author Malin Flodin
 */
public class ClosingForcedState implements AdministrativeState {
    ...
    public ClosingForcedState(CallManagerController cmController,
    ...
    public synchronized void closeForced(CloseForcedEvent closeForcedEvent) {
        if (log.isInfoEnabled())
            log.info("Close request (forced) received in " + this +
                    ". It is ignored.");
    }
    ...
    public synchronized void removeCall(String id) {
        ...
        if (activeCalls > 0) {
            ...
            if (log.isInfoEnabled()) log.info("All calls have been disconnected in " + this +
                                              ". The state is considered Locked.");
