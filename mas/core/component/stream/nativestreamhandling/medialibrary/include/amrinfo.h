...
class MEDIALIB_CLASS_EXPORT AmrInfo: public MediaInfo
{
public:
    AmrInfo(JNIEnv* env);
    ~AmrInfo();
    ...
    int getFramesPerSample() const;
    int getAudioChunkCount() const;
    int getModeSet() const;
	unsigned getFrameSize(unsigned mode);
	...
	virtual const unsigned get_noData(); //returns the value of noData for this type of AMR
	...
	*/ 
    ...
    AmrTrackInfo* getHintTrack();
    AmrTrackInfo* getAudioHintTrack();
    AmrTrackInfo* getVideoTrack();
    AmrTrackInfo* getAudioTrack();

    AmrTrackInfo m_audioTrack;
    AmrTrackInfo m_videoTrack;
    AmrTrackInfo m_hintTrack;
    AmrTrackInfo m_audioHintTrack;
protected:

	JNIEnv *m_env;	
	static const unsigned FT_NO_DATA;
	static const unsigned FT_SID_NB;
	static const unsigned MAX_FRAME_SIZE_NB;
	static const unsigned SAMPLES_PER_FRAME_20MS_NB;
	
public:
    int m_nOfAudioPackets;
    int m_framesPerSample;
    int m_modeSet;
    const unsigned *modeSize;
		
private:
	static const unsigned nbModeSize[16];
