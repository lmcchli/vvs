...
protected:
	...
	QueueRTCPManager(QueueRTCPManager* queueRTCPManager, 
			 uint32 ssrc,
			 uint32 size = RTPDataQueue::defaultMembersHashSize, 
			 RTPApplication& app = defaultApplication());
	 ...
	 **/
	size_t
	dispatchBYE(const std::string& reason, bool bypassBYEReconsideration = false);
...
private:
	...
	QueueRTCPManager* inboundQueueRTCPManager;
};
...
protected: 
	...
	AVPQueue(QueueRTCPManager* queueRTCPManager, 
		 uint32 ssrc, uint32 size = 
		 RTPDataQueue::defaultMembersHashSize, 
		 RTPApplication& app = defaultApplication()) :
		QueueRTCPManager(queueRTCPManager,ssrc,size,app) 
	{ }
