...
public:
	...
	TRTPSessionBase(QueueRTCPManager* queueRTCPManager,
	                 uint32 ssrc,
			 const InetHostAddress& ia, 
			 tpport_t dataPort, tpport_t controlPort,
			 uint32 membersSize, RTPApplication& app, 
			 RTPDataChannel* d, RTCPChannel* c):
			 //ServiceQueue(inboundSession,ssrc,membersSize,app)
			 ServiceQueue(queueRTCPManager,ssrc,membersSize,app)
	{ 
		build(d,c); 
		isInbound = false; 
	}
	...
	virtual size_t dispatchBYE(const ost::String &str, bool bypassBYEReconsideration = false)
	{
		return QueueRTCPManager::dispatchBYE(str, bypassBYEReconsideration);
	}

	inline virtual
	~TRTPSessionBase()
	{ 
		// Since we are tering down the session then we do not do
		// anylonger BYE Reconsideration
		dispatchBYE("RTP session being destroyed, GNU ccRTP stack finishing.", true);
		if(isInbound) {
	 		endSockets(); 
	 	}
	}
	...
	inline RTCPChannel *getCSO(void)
		{return cso;};
...
private:
	...
	void 
	build(RTPDataChannel* d, RTCPChannel* c)
	{
		dso = d;
		cso = c;
	}
...
protected:
	...
	inline ssize_t takeInDataPacket(void)
		{return TRTPSessionBase<RTPDataChannel,RTCPChannel,ServiceQueue>::takeInDataPacket();}

	inline size_t dispatchBYE(const ost::String &str)
