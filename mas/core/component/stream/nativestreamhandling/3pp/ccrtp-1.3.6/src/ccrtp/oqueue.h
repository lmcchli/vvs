...
public:
	 ...
	 **/
	void
	putData(uint32 stamp, const unsigned char* data = NULL, size_t len = 0, bool isTransfer = false);
...
protected:
	...
	/**
	 * This function is used by the service thread to process
	 * the outgoing packets pending in the sending queue.
	 * All packets with timestamps later than ts will be
	 * dispatched.
	 *
	 * @param ts Packets with timestamp later than this timestamp
         *           - <code>deltaTimestamp</code> will be sent.
	 * @param deltaTimestamp Packets that are ready to be sent 
	 *        within this timestamp interval will be sent.
	 * @return 0 if no packets were sent, 1 otherwise.
	 **/
