...
static char *enableDispatchByeMsg=getenv("DISPATCH_BYE_MSG");
...
QueueRTCPManager::QueueRTCPManager(QueueRTCPManager* queueRTCPManager, uint32 ssrc, uint32 size, RTPApplication& app):
	RTPDataQueue(&ssrc, size),
	RTCPCompoundHandler(RTCPCompoundHandler::defaultPathMTU),
	queueApplication(app)
{
	...
	// allow to start RTCP service once everything is set up
	controlServiceActive = true;
	inboundQueueRTCPManager = queueRTCPManager;
}
...
QueueRTCPManager::takeInControlPacket()
{
	...
	catch( ost::Socket* error )
	{
		...
		if (!cont) {
			// Print error only if it's not a timeout. Timeout are viewed as normal
                        time_t currentTime = time(0);
                        struct tm * timeinfo = localtime (&currentTime);
                        char tbuf[TIME_BUFFER_SIZE] = {0};
                        strftime (tbuf, TIME_BUFFER_SIZE, "%F %T #TZ# ", timeinfo);

                        cout << tbuf << "Caught ost::Socket* exception in QueueRTCPManager::takeInControlPacket: " << error->getErrorString();
                        cout << ", Error number: "<< error->getErrorNumber() << ", System Error Number: " << error->getSystemError();
                        cout << ", System error: " << error->getSystemErrorString() << endl;
                        cout.flush();

                        // Do not send/receive RTCP packets at this time - if not a timeout
			controlServiceActive = false;
			throw;
		} else {
			return; // return if timeout before data recv
		}
	}
	catch( ost::SockException const& error )
	{
		...
		if (!cont) {
                        ...
                        cout << tbuf << "Caught ost::SocketException in QueueRTCPManager::takeInControlPacket: Socket Error number: " << error.getSocketError();
	...
	catch (...)
	{
		...
		controlServiceActive = false;
		cout << tbuf << "Unknown Socket exception" << endl;
		cout.flush();
		throw;
	}
...
{	
	...
	if ( getSendersCount() > 0 &&
		...
		//		std::cerr << "computeRTCPIntercal SR" << std::endl;
...
size_t QueueRTCPManager::dispatchBYE(const std::string& reason, bool bypassBYEReconsideration) {
	...
	// If session is not to be destroyed and more than 50 pariticipants then
        // do BYE reconsideration
	if ( (!bypassBYEReconsideration) && (getMembersCount() > 50)) {
		...
		rtcpWeSent = false;
		rtcpAvgSize =
				(uint16) (sizeof(RTCPFixedHeader) + sizeof(uint32)
						+ strlen(reason.c_str())
		...
		while (timercmp(&(reconsInfo.rtcpTc), &(reconsInfo.rtcpTn), <)) {
			...
			if (enableDispatchByeMsg != NULL) {
				time_t currentTime = time(0);
                                struct tm * timeinfo = localtime (&currentTime);
                                char tbuf[TIME_BUFFER_SIZE] = {0};
                                strftime (tbuf, TIME_BUFFER_SIZE, "%F %T #TZ# ",timeinfo);
				cout << tbuf << "Retry No: " << retry << ". " << reason << endl;
				cout.flush();
			}
...
QueueRTCPManager::getOnlyBye()
{
	...
	do {
		try {
			len = recvControl(rtcpRecvBuffer,getPathMTU(),
				  network_address,transport_port);
				   ...
				   uint16& available)
{
	...
	for ( ;
		...
		if ( NULL == si ) {
			...
			timeval now, diff;
			gettimeofday(&now,NULL);
			timeval l = srcLink.getLastRTCPSRTime();
			timersub(&now,&l,&diff);
			blocks[j].rinfo.dlsr = 
	...
	if((i==NULL)&&(j==0)) {
	        ...
	        if (!(inboundQueueRTCPManager==NULL)) {

	           SyncSourceLink* ii = inboundQueueRTCPManager->getFirst();
