...
{
	if ( NULL == senderInfo )
		senderInfo = new unsigned char[sizeof(RTCPCompoundHandler::SenderInfo)];
...
setReceiverInfo(unsigned char* ri)
{
	if ( NULL == receiverInfo )
		receiverInfo = new unsigned char[sizeof(RTCPCompoundHandler::ReceiverInfo)];
...
MembershipBookkeeping::endMembers()
{
	...
	while( first ) {
		...
		try {
#endif
			removeSource(s->getSource()->getID());
#ifdef	CCXX_EXCEPTIONS
		} catch (...) {}
#endif
	}
	...
	try {
#endif
		// In case anything is left in sourceLinks
	    for ( uint32 i = 0; i < sourceBucketsNum; i++ ) {
		   if (sourceLinks[i]) {
		   	removeSource(sourceLinks[i]->getSource()->getID());
		   }
		}
		delete [] sourceLinks;
		sourceLinks = NULL;
		...
		if ( s->getSource()->getID() == ssrc ) {
			...
			if ( old ) {
				old->setNextCollis(s->getNextCollis());
		             ...
		             sourceLinks[ hashing ] = s->getNextCollis();
			}
