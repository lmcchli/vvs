...
public:
	...
	setSDESItem(Participant* part, SDESItemType item, 
		    const ost::String& val)
	{ part->setSDESItem(item,val); }

	inline void
	setPRIVPrefix(Participant* part, const ost::String val)
...
public:
	...
	struct SyncSourceLink
	{
		...
		inline double getLastPacketTransitTime()
		{ return lastPacketTransitTime; }
		
		inline void setLastPacketTransitTime(double time)
		{ lastPacketTransitTime = time; }

		inline double getJitter() const
		{ return jitter; }

		inline void setJitter(double j)
		{ jitter = j; }
...
{
public:
    using MembershipBookkeeping::getLink;
    using MembershipBookkeeping::getFirst;
    using MembershipBookkeeping::getLast;
