...
#pragma pack(1)	
  ...
  /**
   * @struct PSFB_FIR_FCI_Entry
   *
   * @short Struct for an FCI entry in
   * PSFB_FIR_Packet (see draft-ietf-avt-avpf-ccm)
  **/
  struct PSFB_FIR_FCI_Entry
  {
    uint32 ssrc;
    unsigned char seqNo:8; 
    uint32 reserved:24; // Should be 0
  };

  /**
   * @struct PSFBFIRPacket
   *
   * @short Struct for Payload-specific RTCP feedback
   * FIR packet (see RFC 4585 and draft-ietf-avt-avpf-ccm).
   **/
  struct PSFB_FIR_Packet
  {
    uint32 ssrc;
    uint32 ssrcMediaSource; // should be set to 0 for FIR
    PSFB_FIR_FCI_Entry fciEntries[1];
  };
	...
	struct RTCPPacket
	{
		...
		typedef enum {
			...
			tXR,            ///< Extended Report.
      tPSFB  = 206    ///< Payload-specific feedback
		}       Type;
		
    /**
     * @enum
     *
     * Types of Payload-specific feedback, this is set as
     * the block_count in RTCPFixedHeader.. Described as
     * "FMT" in RFC 4585.
     */
    typedef enum {
      fFIR = 4
