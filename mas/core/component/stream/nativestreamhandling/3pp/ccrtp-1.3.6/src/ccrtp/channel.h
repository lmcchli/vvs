...
#ifndef WIN32
#include <sys/ioctl.h>
inline int ccioctl(SOCKET so, int request, void* arg) 
{ return ::ioctl(so,request,arg); }
#else
inline int ccioctl(SOCKET so, int request, void*arg)
{ 
	unsigned long l = 0; 
...
public:
	...
	{ int len; ccioctl(UDPSocket::so,FIONREAD,&len); return len; }
...
public:
	...
	{
                // I order to support RFC 4961 we have
                // bind the outbound socket to a specific port
                // which is the same as the inbound socket
                // ebermel 2008-09-04
		recvSocket = new BaseSocket(ia,port);
		// sendSocket = new BaseSocket(ia,port);
		sendSocket = new BaseSocket(ia,port);
	}
