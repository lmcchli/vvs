...
ConflictHandler::ConflictingTransportAddress::ConflictingTransportAddress(InetAddress na, tpport_t dtp, tpport_t ctp) :
                networkAddress(na), dataTransportPort(dtp), controlTransportPort(ctp), next(NULL)
{
...
ConflictHandler::searchDataConflict(InetAddress na, tpport_t dtp)
{
    ...
    while (result->networkAddress != na || result->dataTransportPort != dtp)
        result = result->next;
    return result;
}
...
{
    ConflictingTransportAddress* result = firstConflict;
    while (result && (result->networkAddress != na || result->controlTransportPort != ctp))
        result = result->next;
    return result;
}
...
{
    ConflictingTransportAddress* nc = new ConflictingTransportAddress(na, dtp, ctp);
...
{
    // HK47306: ConflictingTransportAddress elements must be deleted.
    while (firstConflict != 0) {
        ConflictingTransportAddress* next = firstConflict->next;
        delete firstConflict;
        firstConflict = next;
    }
}

const uint8 IncomingDataQueue::defaultMinValidPacketSequence = 0;
const uint16 IncomingDataQueue::defaultMaxPacketMisorder = 100;
const uint16 IncomingDataQueue::defaultMaxPacketDropout = 3000;
const size_t IncomingDataQueue::defaultMembersSize = MembershipBookkeeping::defaultMembersHashSize;

IncomingDataQueue::IncomingDataQueue(uint32 size) :
                IncomingDataQueueBase(), MembershipBookkeeping(size), recvLock()
{
...
bool IncomingDataQueue::isWaiting(const SyncSource* src) const
{
    bool w;
    recvLock.readLock();
    if ( NULL == src)
...
uint32 IncomingDataQueue::getFirstTimestamp(const SyncSource* src) const
{
    recvLock.readLock();
...
ssize_t IncomingDataQueue::takeInDataPacket(void)
{
    ...
    ssize_t nextSize = getNextDataPacketSize();
    if (nextSize < 0) {
        return rtn;
    }
    ...
    try {
        rtn = (int32) recvData(buffer, nextSize, network_address, transport_port);
        if (rtn < 0) {
            delete[] buffer;
            return rtn;
        }
        if ((uint32) rtn > getMaxRecvPacketSize()) {
            time_t currentTime = time(0);
            struct tm * timeinfo = localtime (&currentTime);
            char tbuf[TIME_BUFFER_SIZE] = {0};
            strftime (tbuf, TIME_BUFFER_SIZE, "%F %T #TZ# ",timeinfo);

            cout << tbuf << "ccRTP: Dropping huge packet with size : " << rtn << " bytes long." << endl;
            cout.flush();
        ...
        IncomingRTPPkt* packet = new IncomingRTPPkt(buffer, rtn);
        ...
        if (!packet->isHeaderValid()) {
            ...
            cout << tbuf << "ccRTP: Dropping packet with size " << rtn << " due to bad header." << endl;
            cout.flush();
            delete packet;
            return 0;
        }
        ...
        if (checkSSRCInIncomingRTPPkt(*sourceLink, source_created, network_address, transport_port)
        ...
        bool cont = ((error->getSystemError() == EAGAIN) || (error->getSystemError() == EWOULDBLOCK));
        if (!cont) {
            time_t currentTime = time(0);
            struct tm * timeinfo = localtime (&currentTime);
            char tbuf[TIME_BUFFER_SIZE] = {0};
            ...
            cout << tbuf << "Caught ost::Socket* exception in IncomingDataQueue::takeInDataPacket: "
                    << error->getErrorString();
            cout << ", Error number: " << error->getErrorNumber() << ", System Error Number: "
                    << error->getSystemError();
            cout << ", System error: " << error->getSystemErrorString() << endl;
            cout.flush();
        ...
        if (!cont) {
            ...
            cout << tbuf << "Caught ost::SocketException in IncomingDataQueue::takeInDataPacket: Socket Error number: "
                    << error.getSocketError();
            const char* errStr = error.getString();
            if (errStr != NULL) {
                cout << ", Error String: " << errStr;
            }
            cout << ", System Error numer: " << error.getSystemError();
            const char* systErrStr = error.getSystemErrorString();
            if (systErrStr != NULL) {
                cout << ",System error: " << systErrStr;
            }
            cout << endl;
            cout.flush();
...
bool IncomingDataQueue::checkSSRCInIncomingRTPPkt(SyncSourceLink& sourceLink, bool is_new,
        InetAddress& network_address, tpport_t transport_port)

{
    ...
    if (s->getDataTransportPort() != transport_port || s->getNetworkAddress() != network_address) {
        ...
        if (s->getID() != getLocalSSRC()) {
            ...
            if (sourceLink.getPrevConflict()
                    && ((network_address == sourceLink.getPrevConflict()->networkAddress)
                            && (transport_port == sourceLink.getPrevConflict()->dataTransportPort))) {
                ...
                // collision repeats.
                sourceLink.setPrevConflict(network_address, transport_port, 0);
            ...
            ConflictingTransportAddress* conflicting = searchDataConflict(network_address, transport_port);
            if (conflicting) {
                ...
                // New collision
                addConflict(s->getNetworkAddress(), s->getDataTransportPort(), s->getControlTransportPort());
...
IncomingDataQueue::getWaiting(uint32 timestamp, const SyncSource* src)
{
    ...
    if (src != NULL) {
        ...
        while (l && ((l->getTimestamp() < timestamp) || end2EndDelayed(*l))) {
            nold++;
            l = l->getSrcNext();
        }
...
bool IncomingDataQueue::recordReception(SyncSourceLink& srcLink, const IncomingRTPPkt& pkt, const timeval recvtime)
{
    bool result = true;
    ...
    if (!(srcLink.isValid())) {
        ...
        if (step < getMaxPacketDropout()) {
            ...
            if (pkt.getSeqNum() == srcLink.getBadSeqNum()) {
                ...
                srcLink.setBadSeqNum((pkt.getSeqNum() + 1) & (SEQNUMMOD - 1));
    ...
    if (result) {
        ...
        if (srcLink.getObservedPacketCount() > 1) {
            ...
            double srTimestampSec = (((static_cast<double>(pkt.getTimestamp()))
                    / (static_cast<double>(getCurrentRTPClockRate()))) * (1000000.0f));

            // Since Cisco, Wireshark and Kapanga divide by sample rate, same will be done here
            //double transitTimeSec = (static_cast<double>(arrival)-srTimestampSec)*((static_cast<double>(getCurrentRTPClockRate()))/(1000000));
            double transitTimeSec = (static_cast<double>(arrival) - srTimestampSec) / (1000000.0f);

            double deltaFloat = 0.0f;

            if (transitTimeSec < 0.0f)
                transitTimeSec = -transitTimeSec;

            if (srcLink.getObservedPacketCount() > 2) {
                deltaFloat = transitTimeSec - srcLink.getLastPacketTransitTime();
            }

            srcLink.setLastPacketTransitTime(transitTimeSec);

            if (srcLink.getObservedPacketCount() > 2) {
                if (deltaFloat < 0)
                    deltaFloat = -deltaFloat;

                srcLink.setJitter(srcLink.getJitter() + (1.0f / 16.0f) * ((deltaFloat) - srcLink.getJitter()));
...
void IncomingDataQueue::recordExtraction(const IncomingRTPPkt&)
{
}
