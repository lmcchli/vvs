...
Socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP)
{
	...
	{
		endSocket();
                const char *tmp = "Could not bind socket";
		error(errBindingFailed,tmp,socket_errno);
		return;
	}
	state = BOUND;
	setError(false);
...
SocketService::~SocketService()
{
	...
	while(first) {
		...
		// I have no idea why delete was put in a while loop.
		// Since there might be something I do not quite understand
		// I will leave it in the loop. The delete operator does not
		// set the pointer to NULL. I just added 'first = null'.
		delete first;
	        first = NULL;	
         }
}
