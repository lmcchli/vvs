...
Socket::Socket(int domain, int type, int protocol)
{
	...
	if(so == INVALID_SOCKET)
	{
		const char *tmp="Could not create socket";
		error(errCreateFailed,tmp,socket_errno);
		return;
	}
#ifdef	SO_NOSIGPIPE
	int opt = 1;
...
Socket::Socket(SOCKET fd)
{
	...
	{
		const char *tmp="Invalid socket handle passed";
		error(errCreateFailed,tmp,0);
		return;
	}
	so = fd;
	state = AVAILABLE;
...
Socket::Socket(const Socket &orig)
{
	...
	if(so == INVALID_SOCKET)
	{
		const char *tmp = "Could not duplicate socket handle";
		error(errCopyFailed,tmp,socket_errno);
	}
	state = orig.state;
}
...
ssize_t Socket::readLine(char *str, size_t request, timeout_t timeout)
{
	...
	while(nleft && !nl)
	{
		if(timeout)
		{
			...
			{
				const char *tmp = "Read timeout";
				error(errTimeout,tmp, 0);
				return -1;
			}
		}
		...
		if(nstat <= 0)
		{
			const char *tmp = "Could not read from socket";
			error(errInput,tmp, socket_errno);
			return -1;
		}
...
Socket::Error Socket::error(Error err, const char *errs, long systemError) const
{
	...
	case Thread::throwException:
		{
			if(!errs)
				throw SockException(String(""), err, systemError);
			else
...
#ifdef WIN32
	int rc = ::recvfrom(so, &buf, 0, MSG_PEEK,
...
Socket::Error UDPSocket::join(const IPV4Multicast &ia,int InterfaceIndex)
{
	...
	struct ip_mreqn      group;
        ...
        getsockname(so, (struct sockaddr *)&myaddr, &len);
	memset(&group, 0, sizeof(group));
        memcpy(&group.imr_address, &myaddr.sin_addr, sizeof(struct in_addr));
        group.imr_multiaddr = getaddress(ia);
