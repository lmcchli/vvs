...
/**
 * The masServiceEnablerStatisticsEntry family
 */
public final class k_masServiceEnablerStatisticsEntry extends v_masServiceEnablerStatisticsEntry {
    private V2Table serviceEnablerStatisticsTable = new V2Table();
	...
	if (attrs != null) {
	    if (attrs != cache) {
		cache = attrs;
		serviceEnablerStatisticsTable = new V2Table();
		for (int i = 0; i < attrs.serviceEnablers.size(); ++i) {
		    /* --- Construct a table entry --- */
		    MasMibServiceEnabler mmse = attrs.serviceEnablers.get(i);
		    k_masServiceEnablerStatisticsEntry mses = new k_masServiceEnablerStatisticsEntry();

		    /* --- Populate the new table entry --- */
		    mses.mibObjects[I_masServiceEnablerStatisticsIndex] = new Integer32(i + 1);
		    mses.mibObjects[I_masServiceEnablerStatisticsProtocol] = new OctetString(mmse.protocol);
		    mses.mibObjects[I_masServiceEnablerStatisticsMaxConnections] = new Integer32((int) mmse.maxConnections);

		    /* --- Add the table entry to the table --- */
		    serviceEnablerStatisticsTable.addRow(mses);
		}
	    }
	} else {
	    cache = null;
     ...
     *                    masServiceEnablerStatisticsEntry object should be retrieved
	...
	k_masServiceEnablerStatisticsEntry result;
	k_masServiceEnablerStatisticsEntry mses = new k_masServiceEnablerStatisticsEntry();
	...
	mses.mibObjects[I_masServiceEnablerStatisticsIndex] = index[0];

	/* --- Extract the entry from the table --- */
	result = (k_masServiceEnablerStatisticsEntry) serviceEnablerStatisticsTable.getRow(mses, searchType);
