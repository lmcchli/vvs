test_description="SIP - GTD support - Get Redirect Reason (unknown) from Diversion part of SIP INVITE (even though Deflection is in GTD)"
test_id="20010105"
function test_func {
...
#	make-sipp-call emarfro_testcases/100.xml $calling $called $redir
	...
	tail -1 ./${base_filename}$$.tmp | grep "This is the C-reason: unknown" > /dev/null
	if [ $? -eq 0 ]; then
