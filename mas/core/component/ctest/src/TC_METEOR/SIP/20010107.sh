test_description="SIP - GTD support - Get Redirect Reason (mobile subscriber not reachable) from GTD part of SIP INVITE (when Unknown in Diversion)"
test_id="20010107"
function test_func {
...
#	make-sipp-call emarfro_testcases/100.xml $calling $called $redir
	...
	tail -1 ./${base_filename}$$.tmp | grep "This is the C-reason: mobile subscriber not reachable" > /dev/null
	if [ $? -eq 0 ]; then
