test_description="SIP - GTD support - Get Redirect Reason (unconditional) from Diversion part of SIP INVITE"
test_id="20010104"
function test_func {
...
#	make-sipp-call emarfro_testcases/100.xml $calling $called $redir
	...
	tail -1 ./${base_filename}$$.tmp | grep "This is the C-reason: unconditional" > /dev/null
	if [ $? -eq 0 ]; then
