...
test_description="Verify that MAS will send the 180 message as a reliable response if early media is not used by the service application and the invite includes a required header with 100rel and sendprovisionalresponsesreliable is set to \"sdponly\""
test_id="11"
...
function test_func {
	...
	echo -n "Checking call trace for: \"Require: 100rel\"... for 180 message " >> $temp_log
	...
	rm -f trace.pcap
	# Restore the original mas.xml file
