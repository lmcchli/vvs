...
test_description="Verify that MAS will send the 183 message as a reliable response if early media is used by the service application and the invite includes a supported header with 100rel and sendprovisionalresponsesreliable is set to \"yes\""
test_id="5"
...
function test_func {
	...
	echo -n "Checking call trace for: \"Supported: 100rel\"... " >> $temp_log
	grep -a 183 trace.pcap | grep "Supported: 100rel" > /dev/null
	if [ $? -eq 0 ]; then
