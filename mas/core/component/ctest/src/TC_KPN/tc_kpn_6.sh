...
test_description="Verify that MAS will send the 180 message as a reliable response if early media is not used by the service application and the invite includes a supported header with 100rel and sendprovisionalresponsesreliable is set to \"yes\""
test_id="6"
...
function test_func {
	...
	# Install the required MAS configuration files
	install-mas-configuration $prefix_dir/mas.xml.sprr_yes
	# Reload the MAS configuration
	mas-reload-config

	# Start a call trace
	start_call_trace trace.pcap "host jokkfall-2.lab.mobeon.com and port 5060"
	...
	echo -n "Checking MAS log for: \"...SIP 180 response is sent reliably...\"... " >> $temp_log
	cat ./${base_filename}$$.tmp | \
		grep "$mas_session" | \
		grep "SIP 180 response is sent reliably" > /dev/null
	if [ $? -eq 0 ]; then
		echo "PASS" >> $temp_log; let passed++
	else
		echo "FAIL" >> $temp_log; let failed++
