...
test_description="Verify that MAS handles renegotiations correctly and sends the 183 message as a reliable response if early media is used by the service application and the invite includes a supported header with 100rel and sendprovisionalresponsesreliable is set to \"sdponly\""
test_id="16"
...
function test_func {
	...
	install-mas-configuration $prefix_dir/mas.xml.sprr_sdponly
	...
	# Verify SDP response in the 200 OK response
	echo -n "Verifying SDP response in the 200 OK response..." >> $temp_log
	prack_branch=`grep -a "PRACK sip" trace.pcap | cut -d ';' -f2 | awk -F'From' '{print $1}'`
	grep -a "SIP\/2\.0 200" trace.pcap | \
		grep -a -v $prack_branch |
		grep -a "Supported: 100rel" > /dev/null
	if [ $? -eq 0 ]; then
		echo "PASS" >> $temp_log; let passed++
	else
		echo "FAIL" >> $temp_log; let failed++
