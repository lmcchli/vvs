...
test_description="Verify that MAS will send the 183 message as a non-reliable response if early media is used by the service application and the invite includes a supported header with 100rel and sendprovisionalresponsesreliable is set to \"no\""
test_id="18"
...
function test_func {
	...
	echo -n "Ensuring call trace doesn't contain: \"Require: 100rel\"... for 183 message..." >> $temp_log
	...
	echo -n "Checking MAS log for: \"...SIP 183 response is sent for...\"... " >> $temp_log
	cat ./${base_filename}$$.tmp | \
		grep "$mas_session" | \
		grep "SIP 183 response is sent for" > /dev/null
	if [ $? -eq 0 ]; then
		echo "PASS" >> $temp_log; let passed++
	else
		echo "FAIL" >> $temp_log; let failed++
