#
# Functions required by the Brutele test suite to install and uninstall
...
if [ -a $prefix_dir/$test_id.vxml.template ]; then
	...
	cp $prefix_dir/../default.vxml.template $prefix_dir/../$test_id.vxml
fi
	scp $prefix_dir/../$test_id.vxml $MAS_USERNAME@$MAS_HOST:$MAS_PATH/applications/ > /dev/null

	# Replace parameters in the CCXML application (if other than default CCXML file exist)
	if [ -a $prefix_dir/$test_id.ccxml.template ]; then
		cp $prefix_dir/$test_id.ccxml.template $prefix_dir/../default.ccxml.$test_id
	else
