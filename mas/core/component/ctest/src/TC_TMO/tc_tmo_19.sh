...
test_description="Verify that MAS sets the correct reason for the redirection when the redirection reason is 404"
test_id="19"
...
function test_func {
	...
	/usr/ucb/echo -n "Found cause = \"<Redirecting Party = ...<Cause = unknown (404)>>\" ... " >> $temp_log
	tail -1 ./${base_filename}$$.tmp | \
		awk -F, '{ print $15 }' | \
		sed -e 's/ <Cause = //g' | \
		sed -e 's/>>//g' | \
		grep "unknown (404)" > /dev/null
	if [ $? -eq 0 ]; then
		echo "PASS" >> $temp_log; let passed++
    else
