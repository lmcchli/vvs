...
test_description="Verify that MAS will use the service abcd if the request URI is abcd"
test_id="55"
...
function test_func {
	...
	/usr/ucb/echo -n "Found \"Trying to load service based on called party: abcd\"... " >> $temp_log
	grep "Trying to load service based on called party: abcd" ./${base_filename}$$.tmp > /dev/null
	if [ $? -eq 0 ]; then
