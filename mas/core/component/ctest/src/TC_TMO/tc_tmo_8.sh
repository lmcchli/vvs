...
test_description="MAS will use the value from the P-Asserted-Identity header"
test_id="8"
...
function test_func {
	...
	/usr/ucb/echo -n "evt.connection.remote.number resulted in '$header_p_asserted_id'... " >> $temp_log
	get-mas-log-and-filter "PhoneNumber_GetAnalyzedNumber.*evt.connection.remote.number"
	tail -1 ./${base_filename}$$.tmp | grep "resulted in '$header_p_asserted_id'" > /dev/null
	if [ $? -eq 0 ]; then
