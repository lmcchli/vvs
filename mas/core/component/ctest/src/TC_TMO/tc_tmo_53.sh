...
function test_func {
	...
	# Check for "Trying to load service based on called party: xyz"
	/usr/ucb/echo -n "Found \"Trying to load service based on called party: xyz\"... " >> $temp_log
	grep "Trying to load service based on called party: xyz" ./${base_filename}$$.tmp > /dev/null
	if [ $? -eq 0 ]; then
	...
	# Check for "Trying to load service based on default service name: Undefined"
	/usr/ucb/echo -n "Found \"Trying to load service based on default service name: Undefined\"... " >> $temp_log
	grep "Trying to load service based on default service name: Undefined" ./${base_filename}$$.tmp > /dev/null
	if [ $? -eq 0 ]; then
		echo "PASS" >> $temp_log; let passed++
    else
		echo "FAIL" >> $temp_log; let failed++
