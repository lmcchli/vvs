...
#TESTCASES=`find TC_MOBINIL/UCIP -name "*.sh"`
#for i in $TESTCASES; do
#	./starfish_engine.sh $i | tee -a $RESULT_LOG
#done

	./starfish_engine.sh TC_MOBINIL/PlatformAccess/20006010.sh | tee -a $RESULT_LOG
	./starfish_engine.sh TC_MOBINIL/UCIP/555.sh | tee -a $RESULT_LOG
	./starfish_engine.sh TC_MOBINIL/UCIP/666.sh | tee -a $RESULT_LOG
	./starfish_engine.sh TC_MOBINIL/UCIP/777.sh | tee -a $RESULT_LOG
	./starfish_engine.sh TC_MOBINIL/UCIP/888.sh | tee -a $RESULT_LOG
	./starfish_engine.sh TC_MOBINIL/UCIP/999.sh | tee -a $RESULT_LOG
