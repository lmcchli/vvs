test_description="UCIP - UpdateBalanceAndDate testcase-collection"
test_id="777"
function test_func {
...
test_id="20016001"
while [  $test_id -lt 20016007 ]; do
  
  if [ $test_id = "20016001" ]; then
  	test_description="UCIP - UpdateBalanceAndDate - adjustmentAmountRelative (+23)"
  elif [ $test_id = "20016002" ]; then
  	test_description="UCIP - UpdateBalanceAndDate - adjustmentAmountRelative (-23)"
  elif [ $test_id = "20016003" ]; then
  	test_description="UCIP - UpdateBalanceAndDate - supervisionExpiryDateRelative (+3)"
  elif [ $test_id = "20016004" ]; then
  	test_description="UCIP - UpdateBalanceAndDate - supervisionExpiryDateRelative (-4)"
  elif [ $test_id = "20016005" ]; then
  	test_description="UCIP - UpdateBalanceAndDate - serviceFeeExpiryDateRelative (+3)"
  elif [ $test_id = "20016006" ]; then
  	test_description="UCIP - UpdateBalanceAndDate - serviceFeeExpiryDateRelative (-4)"
  else
