...
public class MultimediaDistributionListNoSQL extends AbstractMediaFileNoSQL implements IMultimediaDistributionList {
    ...
    private List<String> members = new Vector<String>();    // A list of tel identities from the shortlist
    private static final ILogger logg = ILoggerFactory.getILogger(MultimediaDistributionListNoSQL.class);
    private static final HostedServiceLogger log = new HostedServiceLogger(logg);

    public MultimediaDistributionListNoSQL(String listmsid, int listid, String aUserMsid, IDirectoryUpdater anUpdater) throws Exception {
        super(new MSA(aUserMsid), DistributionList.DISTRIBUTIONLIST_DIR, String.valueOf(listid), "3gp");
     ...
     * @throws IOException
     */
    public MultimediaDistributionListNoSQL(String aUserMsid, IDirectoryUpdater anUpdater) throws Exception {
        super(new MSA(aUserMsid), DistributionList.DISTRIBUTIONLIST_DIR, "");
    ...
    protected void initializeMembers(int anId){
        ...
        try {
            ...
            if(listmsid!=null){
                ...
                if (log.isDebugEnabled()) {
                    log.debug("initializeMembers() No list associated with id "+anId);
                }
            }
    ...
    public void manipulateListMember(Modification.Operation op, String listMember) throws DistributionListException {
        ...
        if (this.listmsid == null || this.shortlistUri == null) {
    ...
    /**
     * Creates a new shortlist profile with the provided {@code anId} in the database and updates
     * the subscriber profile's MOIPDistributionList with this new shortlist.
     *
     * @param the ID of the new shortlist.
    ...
    public String create(int anId) throws DistributionListException {
        ...
        if ( (anId < 1) || (anId > MAX_DISTRIBUTIONLISTS)){
            String m = "The id " + anId + "is out of range, so returning error immediately";
            log.error("create()  " + m);
            throw new DistributionListException(m);
        }
        if (log.isDebugEnabled()){
    ...
    /**
     * Removes the shortlist with the provided {@code andId} from subscriber profile's
     * MOIPDistributionList.
     *
     * @param the ID of the new shortlist.
     * @return just an empty string.
     */
    ...
    /**
     * Updates the subscriber's MOIPDistributionList attribute by adding to it a new shortlist msid and it's integer ID.
     * (ex : "msid:id").
     *
     * @param subscriberTelURI
     * @param listMsid
     * @param anId
    ...
    /**
     * Creates a muid identity from the provided {@code aUserTel} and {@code aListId}.<p>
     *
     * For example, if {@code aUserTel} = tel:5551112222 and {@code aListId} = 4,
     * then the resulting string is muid:5551112222_4
     *
     * @param aUserTel  a tel identity, with or without a "tel:" prefix.
     * @param aListId   a list id
     * @return  the muid identity built from the provided input.
     */
    ...
    /**
     * Creates a shortlist identity from the provided {@code aUserTel} and {@code aListId}.<p>
     *
     * For example, if {@code aUserTel} = tel:5551112222 and {@code aListId} = 4,
     * then the resulting string is shortlist:5551112222_4
     *
     * @param aUserTel  a tel identity, with or without a "tel:" prefix.
     * @param aListId   a list id
     * @return the shortlist identity built from the provided input.
     */
