...
## ntf_ct_4001039.sh --
##
##. 4001039	SMS	Sourceaddress	fax
##. 
##.    Verify that ntf can have different sourceaddresses per type.
##. 
##.    Action: Set sourceaddress_fax to "0,0,fax" in
##.    notification.cfg. Restart ntf. Send an fax message that will get an
##.    notification.
##. 
##.    Result: Check the sms-c to see that the values of the notification
##.    follows the ones set in notification.cfg.
##
## Syntax;
##
##  ntf_ct_4001039.sh cfg-file
##
## Description;
##
...
ssh $ntf_host "echo sourceaddress_fax=0,0,fax >> $ntfcfg"
restart_ntf norestore

# Execute the test
echo "Execute test (wait 5sec) ..."
$send -t fax -c 123400 $phonenumber
wait_for_sms
...
grep "^From: 0,0,fax" $smslog > /dev/null || \
    die "Invalid From: field"
grep "^Data coding: 0" $smslog > /dev/null || die "Invalid Data coding"
grep "^Short message : You have a new fax message regarding" $smslog \
