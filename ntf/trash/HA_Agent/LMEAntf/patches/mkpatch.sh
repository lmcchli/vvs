...
# 1_ntfha_r11b.crh109127.solaris8.tar
#
#
#
...
get_config() {
    PARAMETER=$1
    echo "`cat $HAHOME/patches/mkpatch.cfg | grep $PARAMETER= | sed 's%'$PARAMETER'=\(.*\)%\1%'`"
}
...
set_parameters() {
    ...
    VOB=/vobs/ipms/ntf
    HAHOME=$VOB/HA_Agent/LMEAntf
    PATCHDIR=$HAHOME/patches/BuildPatchFiles
    PKG=`get_config PKG`
    ...
    BUILDDIR=$HAHOME/build/$PATCHID
    PATCHFILE=$HAHOME/delivery/${PATCHID_LOWER}.crh109127.solaris8.zip
...
compile(){
    echo
    echo "**************************************************"
    echo "Starting compilation of NTF HA. Please wait..."
    cd $HAHOME/src
    make clean
    make all
    if [ $? != 0 ]; then
        echo "ERROR WHEN COMPILING NTF HA agent."
        exit 1
    fi
    echo ""
    echo "Done compiling."
...
move_readme(){
    ...
    README_FILE=$HAHOME/delivery/README_${version_without_buildnr}.crh109127.pa1.txt
    cp $HAHOME/patches/BuildPatchFiles/README_TEMPLATE $README_FILE
...
lable_files() {
    ...
    if [ "$SHOULD_LABEL" = "y" ];then
        ...
        echo "\nAttaching label $PATCHID on all files in $HAHOME. Please wait... \c"
...
echo "**************************************************"
echo "COMPLETED."
echo "File is ready for pickup at $HAHOME/delivery/"
exit 0
}
